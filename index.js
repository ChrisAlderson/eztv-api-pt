...
var _typeof = typeof Symbol === "function" && typeof Symbol.iterator === "symbol" ? function (obj) { return typeof obj; } : function (obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol ? "symbol" : typeof obj; }; // Import the neccesary modules.
...
var EZTV = function EZTV() {
  ...
  var getAllShows = function getAllShows() {
    var retry = arguments.length <= 0 || arguments[0] === undefined ? true : arguments[0];

    return new Promise(function (resolve, reject) {
      request("showlist/", function (err, res, body) {
        if (err && retry) {
          ...
          var _ret = function () {
            ...
            $(".thread_link").each(function () {
              var show = $(this).text();
              var id = $(this).attr("href").match(/\/shows\/(.*)\/(.*)\//)[1];
              var slug = $(this).attr("href").match(/\/shows\/(.*)\/(.*)\//)[2];
   ...
   */
  var getShowData = function getShowData(data) {
    var retry = arguments.length <= 1 || arguments[1] === undefined ? true : arguments[1];

    return new Promise(function (resolve, reject) {
      request("shows/" + data.id + "/" + data.slug + "/", function (err, res, body) {
        if (err && retry) {
          ...
          var _ret2 = function () {
            ...
            var imdb = $("div[itemtype='http://schema.org/AggregateRating']").find("a[target='_blank']").attr("href");
            if (imdb) {
            ...
            $("tr.forum_header_border[name='hover']").each(function () {
              var title = $(this).children("td").eq(1).text().replace("x264", "");
              var magnet = $(this).children("td").eq(2).children("a.magnet").first().attr("href");
              if (magnet === null) return true;

              var seasonBased = /S?0*(\d+)?[xE]0*(\d+)/;
              var dateBased = /(\d{4}).(\d{2}.\d{2})/;
              var vtv = /(\d{1,2})[x](\d{2})/;
